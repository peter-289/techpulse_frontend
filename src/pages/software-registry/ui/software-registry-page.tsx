...
export function SoftwareRegistryPage({ scope = 'my-software' }: { scope?: 'my-software' | 'discover' }) {
  const navigate = useNavigate();
  const [query, setQuery] = useState('');
  const [category, setCategory] = useState('all');
  const { data, isLoading, isError, error, refetch, isFetching } = useSoftwareList(120);

  const isDiscover = scope === 'discover';
  const title = isDiscover ? 'Discover software' : 'My software';
  const description = isDiscover ? 'Browse public packages available for discovery and download.' : 'Review your software inventory and release details.';
  ...
  const filtered = useMemo(() => {
    return (data ?? []).filter((pkg) => {
      const matchesScope = isDiscover ? pkg.is_public : true;
  ...
  }, [data, query, category, isDiscover]);

  const totals = useMemo(() => {
    const rows = data ?? [];
    const visibleRows = isDiscover ? rows.filter((pkg) => pkg.is_public) : rows;
    return {
      public: visibleRows.filter((pkg) => pkg.is_public).length,
      paid: visibleRows.filter((pkg) => Number(pkg.price_cents || 0) > 0).length,
      downloads: visibleRows.reduce((sum, pkg) => sum + Number(pkg.download_count || 0), 0),
    };
  }, [data, isDiscover]);
    ...
    <section className="space-y-4">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold text-stone-50">{title}</h1>
      ...
      {isError && (
        <Card className="border-rose-500/50">
          <h2 className="text-base font-medium text-red-200">{isDiscover ? 'Discovery unavailable' : 'Software list unavailable'}</h2>
          <p className="mt-1 text-sm text-stone-300">{error instanceof Error ? error.message : 'Unexpected error while loading software.'}</p>
        ...
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-3">
            <Card className="border-l-4 border-l-teal-400">
              <p className="text-xs text-stone-400">Visible packages</p>
